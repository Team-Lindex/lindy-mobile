...
export default function DailyScreen() {
  ...
  const [currentTwinIndex, setCurrentTwinIndex] = useState(2); // Start with Wendy (index 2)
  ...
  const rotateTwin = () => {
    // Fade out, change twin, then fade in
    Animated.sequence([
      Animated.timing(twinFadeAnim, {
        toValue: 0,
        duration: 200,
        useNativeDriver: true,
      }),
    ...
    // Change the twin index after the fade out starts
    setTimeout(() => {
      setCurrentTwinIndex((prevIndex) => 
        (prevIndex + 1) % twins.length
      );
    }, 200);
  };

  const currentTwin = twins[currentTwinIndex];
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <Text style={styles.twinButtonText}>{currentTwin.name}</Text>
      </TouchableOpacity>
